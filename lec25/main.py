...
def home():
    ...
    last = math.ceil(len(posts)/int(params["num-of-posts"]))
    # pagination logic
    ...
    page = int(page)
    posts = posts[(page-1)*int(params["num-of-posts"]):(page-1)*int(params["num-of-posts"])+params["num-of-posts"]]

    if page == 1:
        prev= "#"
        nxt = f"/?page={str(page+1)}"
    elif page == last:
        prev = f"/?page={str(page-1)}"
        nxt = "#"
    else:
        prev = f"/?page={str(page-1)}"
        nxt = f"/?page={str(page+1)}"
...
def uploader():
    if "user" in session and session["user"] == params["user-name"]:
        if request.method == "POST":
            img_blob = request.files["file1"]
            img_blob.save(os.path.join(app.config["UPLOAD_FOLDER"], secure_filename(img_blob.filename)))
